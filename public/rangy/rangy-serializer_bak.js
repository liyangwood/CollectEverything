...
rangy.createModule("Serializer", function(api, module) {
    ...
    function filterNodeValue(value){
        return value.replace(/<cf (?:.|\s)*?>/g, '').replace(/<\/cf>/g, '');
    }
    ...
    function nodeToInfoString(node, infoParts) {
        ...
        //if(node.getElementsByTagName && node.getElementsByTagName('cf').length > 0){
        //    nodeInfo = [nodeType, node.nodeName, childCount-node.getElementsByTagName('cf').length*2].join(":");
       // }
        ...
        for (var i = 0; i < childCount; ++i) {
            if(children[i].nodeName.toLowerCase() == 'cf'){
                tmp -= 2;
            }
        }
        if(tmp < childCount){
            nodeInfo = [nodeType, node.nodeName, tmp].join(":");
        }
        ...
        switch (nodeType) {
            case 3: // Text node
                start = escapeTextForHtml(filterNodeValue(node.nodeValue));
                break;
            case 8: // Comment
                start = "<!--" + escapeTextForHtml(filterNodeValue(node.nodeValue)) + "-->";
                break;
            default:
                if(node.nodeName.toLowerCase() == 'cf'){
                    break;
                }
        ...
        for (var i = 0; i < childCount; ++i) {
            if(children[i].nodeName.toLowerCase() == 'cf') continue;
    ...
    function serializePosition(node, offset, rootNode) {
        ...
        while (n && n != rootNode) {
            if(n.getElementsByTagName && n.getElementsByTagName('cf').length > 0 && false){
                alert('----'+dom.getNodeIndex(n, true))
                pathBits.push(dom.getNodeIndex(n, true) - n.getElementsByTagName('cf').length*2);
            }
            else{
